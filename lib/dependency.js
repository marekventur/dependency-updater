...
module.exports = class Dependency {
    ...
    generateChangelog(changelog) {
        // Wrap in ES6 Promise to ensure it's not some other Promise implementation
        return Promise.resolve()
        .then(() => changelog.generate(this.name))
        .then(changes => {
            changes.versions = changes.versions.filter(version => semver.outside(version.version, this.version, ">"));
